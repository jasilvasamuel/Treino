...
function saveImage(exId, dataUrl) {
  try {
    ...
    alert(
      'Nao foi possivel salvar (espaco insuficiente).

' +
      'Imagens ja salvas: ~' + mb + ' MB.

' +
      'Dica: remova a imagem de outro exercicio antes de adicionar esta,
' +
      'ou use um GIF mais comprimido (abaixo de 1 MB).'
    );
    return false;
  }
}
...
// ---- DEBUG PANEL (remover apos confirmar que funciona) ----
const debugDiv = document.createElement('div');
debugDiv.id = 'debug-panel';
debugDiv.style.cssText = 'position:fixed;bottom:0;left:0;right:0;background:#000;color:#0f0;font-family:monospace;font-size:11px;padding:8px;max-height:160px;overflow-y:auto;z-index:9999;border-top:2px solid #0f0;';
debugDiv.innerHTML = '<b>DEBUG</b> -- aguardando acao...<br>';
document.body.appendChild(debugDiv);
function dbg(msg) {
  debugDiv.innerHTML += new Date().toLocaleTimeString() + ' ' + msg + '<br>';
  debugDiv.scrollTop = 9999;
}
dbg('JS carregado. localStorage: ' + localStorage.length + ' chaves');
dbg('img-file-input encontrados: ' + document.querySelectorAll('.img-file-input').length);

// File input handler
document.querySelectorAll('.img-file-input').forEach(inp => {
  // Garante que o input seja clicavel diretamente
  // (alguns Android Chrome ignoram label wrapping)
  const lbl = inp.closest('label');
  if (lbl) {
    lbl.addEventListener('click', e => {
      e.preventDefault();
      dbg('Label clicado para ex: ' + inp.dataset.exId);
      inp.click();
    });
  }
  ...
  inp.addEventListener('change', e => {
    ...
    dbg('Change disparado. File: ' + (file ? file.name + ' (' + (file.size/1024).toFixed(0) + 'KB)' : 'null'));
    if (!file) return;
    ...
    reader.onload = ev => {
      ...
      dbg('FileReader OK: ' + (dataUrl ? (dataUrl.length/1024).toFixed(0)+'KB, '+dataUrl.substring(5,20) : 'null'));
      if (!dataUrl || !dataUrl.startsWith('data:')) {
        alert('Erro ao ler o arquivo.');
        return;
      }
      applyCustomImage(exId, dataUrl);
      dbg('applyCustomImage chamado');
      const ok = saveImage(exId, dataUrl);
      dbg('saveImage: ' + (ok ? 'OK' : 'FALHOU'));
    };
    reader.onerror = e2 => { dbg('FileReader ERRO: ' + e2); };
